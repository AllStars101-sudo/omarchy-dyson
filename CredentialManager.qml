...
//
// Adapted from konradk/hass (MIT, Copyright (c) 2026 Konrad Kruk) — see
// THIRD_PARTY_NOTICES.md. Changes from upstream: the secret-tool `service`
// attribute and label are this plugin's, and the legacy unscoped-token
// migration probe is dropped, since it exists to migrate hass installs from
// before its 0.2 and has no meaning here.
QtObject {
  id: root
  ...
  readonly property string service: "io.github.allstars101-sudo.dyson-air"
  readonly property string label: "Home Assistant (Dyson Air for Omarchy)"
  ...
  property Timer lookupStartTimeout: Timer {
    ...
    onTriggered: {
      ...
      root.failed("Could not start secret-tool to read the token. Is libsecret installed and a keyring running?", origin)
